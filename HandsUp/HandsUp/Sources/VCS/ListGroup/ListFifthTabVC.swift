...
class ListFifthTabVC: ListVC, UITableViewDelegate, UITableViewDataSource {
...
//        
//        let filteredList = HomeList.filter { $0.tag == "취미" }
//        
...
//        self.present(myProfile, animated: true)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
            let filteredList = HomeList.filter { $0.tag == "취미" } // 태그에 맞는 요소만 필터링하여 새로운 배열 생성
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let filteredList = HomeList.filter { $0.tag == "취미" } // 태그에 맞는 요소만 필터링하여 새로운 배열 생성
        print("table View filteredList 취미 성공 및 원소 개수 == \(filteredList.count)")
        return filteredList.count
    }
